...
采用`rebase`指令替代`merge`来合并分支，**变基**会将当前分支上的版本在目标分支上**重新提交**，视图记录中会呈现类似“剪枝嫁接”的效果。
...
变基完成后原始版本的记录会从分支树中消失，但其实仍在版本库中。如果想撤销一次变基，可以通过`reflog`查询操作日志，然后直接重置到早前版本（这种做法同样适用于大部分误操作）。

```bash
git reflog                    ;; 查询变基前的版本
git reset --hard <commit-id>  ;; 直接重置分支
```

> 变基的好习惯：在变基前将**待变基版本**创建一个**临时分支**作为记录备份，等变基确认完成后随时可以删掉，如果变基失败则立刻重置。
...
> 最强大的Git命令之一，可以完成类似“将3次连续提交修改整合成1次后导入新分支”这样的神奇操作，甚至可以用来修改任意历史版本。

`rebase -i`又称**交互式变基**，可以手动选择并更新当前分支上的需要在目标分支上重新提交的版本，允许改写，替换，删除和合并。交互式变基提供了对分支上提交历史完整的控制，常用于并入主分支时整理混乱的版本列表。
...
交互式变基执行期间会顺序读取`git-rebase-todo`文件中的行项：

1. `git status`查看变基流程的状态，通过`git rebase --continue`继续下一项待办，在行项读完前变基不会结束。
2. 遇到修改行项会和普通变基一样停下来，`add`修改内容后通过`git commit --amend`重制提交。
